...
class Options:
    ...
    def audit_sheet_sorter (self): #reads all AP's from chosen Audit Sheet and categories them by their color
     ...
     ##### EXCEL EXPORT #####
        ...
        global sheet_counts
        template = "O:/Customers/Amazon/_Templates/Audit Results Template.xltx"
     ...
     ##### MAIN for LOOP #####    
        ...
        ##### FAIL DICTIONAIRES #####
            ...
            fail_broadcasting = {
                ...
                "3" : (f"{guts_sheet['AC5'].value} {AP_custom_name_broadcasting}"),
    ...
    def save_results(self):   #Pop-up message box, asks if user wants to save sorted date to new excel file/sheet
        result = messagebox.askyesno("AP Illuminator","Do you want to export stats into a results excel?")
        #print(result)
        if result == True:   #if they chose yes, prints out the fail stats and export destination
            ...
            #book_results.save('test_audit_results.xlsx') #saved the information to an excell
            #book_results.save(f"{WHID}.DATE.WAP.SURVEYNAMEResults.CONFIDENTIAL.xlsx")
            book_results.save(f'O:/Customers/Amazon/{WHID}/Validations/{WHID}.DATE.WAP.SURVEYNAMEResults.CONFIDENTIAL.xlsx')
            print(f"Exported results to: 'O:/Customers/Amazon/{WHID}/Validations/\n    {WHID}.DATE.WAP.SURVEYNAMEResults.CONFIDENTIAL.xlsx'\n")  #export destination
            print(f'==============================================================================\n')
        #exit()  
        else:
            pass
...
# Changes *********************
# ******************************
    # Added functionality to use GUTS and make the Audit Results report
    # OPens excel template of audit results: SITE.DATE.WAP.SURVEYNAMEResults.CONFIDENTIAL.xltx
    # copies down all data and exports as separate excel document
    # added exporet button to Coloring GUI for exporting the reeuslts data
    # Fixed GUI button spacing
    # Imporved dictionary functionality and added more values
    # added row/col for reading the Actual Recorded Heights
